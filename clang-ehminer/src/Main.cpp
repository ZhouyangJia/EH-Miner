//===- Main.cpp - driver of the EH-Miner tool-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the entrance of the EH-Miner tool.
//
//===----------------------------------------------------------------------===//
...
#define DEFAULT_CONFIG_FILE "/Users/zhouyangjia/llvm-4.0.0.src/tools/clang/tools/clang-ehminer/etc/test.conf"
...
static cl::extrahelp MoreHelp(
                              "\tFor example, to run clang-ehminer on all files in a subtree of the\n"
                              "\tsource tree, use:\n"
                              "\n"
                              "\t  find path/in/subtree -name '*.cpp'|xargs clang-ehminer\n"
                              "\n"
                              "\tor using a specific build path:\n"
                              "\n"
                              "\t  find path/in/subtree -name '*.cpp'|xargs clang-ehminer -p build/path\n"
                              "\n"
                              ...
                              "\tThe default file is in path/to/clang/tools/clang-ehminer/etc/test.conf.\n"
                              "\n"
                              ...
                              "\tand invoke clang-ehminer sevral times to avoid the command line becoming\n"
                              "\ttoo long, so the data in memory will be lost. To avoid this, use:\n"
                              "\n"
                              "\t  find path/in/subtree -name '*.cpp' > all_files.in\n"
                              "\t  clang-ehminer -p build/path -source-file=all_files.in empty.c\n"
                              "\n"
                              ...
                              "\n"
                              "\t  find path/in/subtree -name '*.cpp'| xargs clang-ehminer -p build/path -database-file=/absolute/path/to/database.db\n"
                              "\n"
                              "\tor use: (there are too many source files)\n"
                              "\n"
                              "\t  find path/in/subtree -name '*.cpp' > all_files.in\n"
                              "\t  clang-ehminer -p build/path -database-file=/absolute/path/to/database.db -source-file=all_files.in empty.c\n"
                              "\n"
                              "\tor use: (anslyze subdirection one by one)\n"
                              "\n"
                              "\t  find path/in/subtree/subdir1 -name '*.cpp'| xargs clang-ehminer -p build/path -database-file=/absolute/path/to/database.db\n"
                              "\t  find path/in/subtree/subdir2 -name '*.cpp'| xargs clang-ehminer -p build/path -database-file=/absolute/path/to/database.db\n"
                              "\n"
                              );

// Deal with command line options
static cl::OptionCategory ClangMytoolCategory("clang-ehminer options");
...
static cl::opt<string> SourceFile("source-file",
                                    cl::desc("Specify source file (default is path/to/clang/tools/clang-ehminer/etc/test.conf."),
                                    cl::cat(ClangMytoolCategory));
...
int main(int argc, const char **argv){
    ...
    if(FindBranchCall){
        ...
        for(unsigned i = 0; i < source.size(); i++){
            ...
            if (access(source[i].c_str(), F_OK)){
                llvm::errs()<<"File doesn't exist: "<<source[i]<<"\n";
                continue;
            }
